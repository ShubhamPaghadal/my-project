...
function Wishlist({add_wishlist, setadd_wishlist}) {
  console.log('add_wishlist222.',add_wishlist)

  const removecart = (id) => {
    const items_filter = add_wishlist.filter((item) => item !== id)
    setadd_wishlist([...add_wishlist, items_filter])
  }

  const price_mapping = add_wishlist.map((cart_price) => cart_price.price).reduce((acc, pre) => acc + pre , 0)
