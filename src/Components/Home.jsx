...
function Home({add_cart,  add_item, add_wishlist_cart, product ,loading}) {
    ...
    const filterProducts = selectedCategory ? product.filter((product) => product.category === selectedCategory.value) : product;
...
/> */}
  ...
  {
    filterProducts <= 0 ? <h1>Product is not found</h1> :
        filterProducts.map((item) => (
        <Cart key={item.id} item = {item } add_cart={add_cart} add_wishlist_cart={add_wishlist_cart} add_item={add_item} remove_cart={remove_cart}/>
    )) 
}
  </div>
