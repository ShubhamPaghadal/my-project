...
function Cart({ item, add_cart, add_item, add_wishlist_cart, remove_cart }) {
  ...
  p-5 rounded-2xl border-solid hover:shadow-[0_4px_8px_rgba(0,0,0,0.1)] shadow-2xl"
    >
      <span className="absolute font-medium text-[rgb(163,37,37)] text-[1.2rem] w-[25px] h-[25px] flex justify-center items-center cursor-pointer transition-[background-color] duration-[0.3s] ease-[ease] rounded-[50%] right-0 top-0 hover:text-[rgb(252,6,6)]">
        <button onClick={() => add_wishlist_cart(item)}>
          {" "}
          <FaRegHeart />
        </button>
      </span>
